...
for i=1:nJts-1
    ...
    for j=i+1:nJts
        ...
        if (isequal(jt_first{1},'R') && isequal(jt_second{1},'R'))
            Constraint=RRconst(jt_first{2},jt_second{2});
        elseif (isequal(jt_first{1},'P') && isequal(jt_second{1},'P'))
            Constraint=PPconst(jt_first{2},jt_second{2});
        elseif (isequal(jt_first{1},'R') && isequal(jt_second{1},'P'))
            Constraint=RPconst(jt_first{2},jt_second{2});
        elseif (isequal(jt_first{1},'P') && isequal(jt_second{1},'R'))
            Constraint=RPconst(jt_second{2},jt_first{2});
        end
        LinkConst=cat(1,LinkConst,Constraint);
    end
end
end
...
dist=(L(1)*P(1)+L(2)*P(2)+L(3))/norm([L(1),L(2)]);
end
